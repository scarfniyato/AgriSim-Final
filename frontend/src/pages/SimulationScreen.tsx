...
export default function SimulationScreen() {
  ...
  const routeState = location.state as
    | { config: SimulationConfig; simulationData?: SimulationApiData }
    | undefined;
  ...
  const [activeEffect, setActiveEffect] = useState<
    "water" | "fertilizer" | "pesticide" | null
  >(null);
  const [overlayAlerts, setOverlayAlerts] = useState<
    {
      id: string;
  ...
  const dayResult: DailySimulationResult | null = useMemo(() => {
    ...
    const idx = Math.max(
      0,
      Math.min(currentDay - 1, simulationData.daily_results.length - 1),
    );
  ...
  const growthStage = dayResult
    ? mapGrowthStageToVisual(dayResult.growth_stage)
    : "Pre-Emergence";

  // Determine if crop has reached maturity at current day
  const isAtMaturity =
    simulationData?.maturity_day != null &&
    currentDay >= simulationData.maturity_day;
  ...
  const finalYield = useMemo(() => {
    ...
    if (simulationData?.yield_estimate_kg_m2 != null)
      return simulationData.yield_estimate_kg_m2;
    return null;
  ...
  const handleJump = useCallback(
    async (days: number) => {
      ...
      if (isResimulating) {
        toast.info(
          "Please wait for the current re-simulation to finish before jumping days.",
        );
        return;
      }
      ...
      const workingIrrigation: Record<string, number> = {
        ...irrigationSchedule,
      };
        ...
        for (let day = currentDay + 1; day <= targetDay; day += 1) {
          ...
          if (autoIrrigateEnabled) {
            ...
            if (!alreadyIrrigatedToday && dayResultForRule.f_water < 1.0) {
              workingIrrigation[dayKey] =
                (workingIrrigation[dayKey] ?? 0) + irrigationMm;
  ...
  const triggerEffect = useCallback(
    (effect: "water" | "fertilizer" | "pesticide") => {
  ...
  const handleWater = useCallback(
    (source: "manual" | "auto" = "manual") => {
      ...
      const updatedSchedule = {
        ...irrigationSchedule,
        [dayKey]: (irrigationSchedule[dayKey] ?? 0) + mm,
      };
  ...
  useEffect(() => {
    if (!isRunning || !dayResult || !config || isResimulating || isAtMaturity)
      return;
          ...
          <div className="max-w-full flex items-center gap-6 text-sm">
            <div className="flex items-center gap-2 flex-shrink-0">
              ...
              <span className="font-semibold text-foreground">
                {getCropDisplayName(config.crop)}
              </span>
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              ...
              <span className="font-semibold text-foreground">
                {getLocationDisplayName(config.location)}
              </span>
            </div>
            <div className="w-px h-4 bg-border" />
            <div className="flex items-center gap-2 flex-shrink-0">
              ...
              <span className="font-semibold text-foreground">
                {config.season === "wet_season" ? "Wet Season" : "Dry Season"}
              </span>
            </div>
            <div className="w-px h-4 bg-border" />
            <div className="flex items-center gap-2 flex-shrink-0">
              ...
              <span className="font-semibold text-foreground">
                {getMonthDisplayName(config.planting_month)}
              </span>
            </div>
            <div className="w-px h-4 bg-border" />
            <div className="flex items-center gap-2 flex-shrink-0">
              ...
              <span className="font-semibold text-foreground">
                {getCO2DisplayName(config.co2_level)}
              </span>
            </div>
            <div className="w-px h-4 bg-border" />
            <div className="flex items-center gap-2 flex-shrink-0">
              ...
              <span className="font-semibold text-foreground">
                {config.fertilizer_level === "none"
                  ? "No Fertilizer"
                  : config.fertilizer_level === "low"
                    ? "Low (50%)"
                    : config.fertilizer_level === "recommended"
                      ? "Recommended"
                      : "High (150%)"}
              </span>
            </div>
            ...
            {simulationData?.maturity_day != null && (
              <>
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  <span className="font-semibold text-foreground">
                    Day {simulationData.maturity_day}
                  </span>
                </div>
              </>
            )}
            ...
            <div className="bg-card rounded-xl border border-border p-4 space-y-2 text-sm">
              ...
              <div className="flex justify-between text-muted-foreground">
                ...
                <span className="font-mono text-foreground">
                  {dayResult.gdd.toFixed(1)} °C·d
                </span>
              </div>
              <div className="flex justify-between text-muted-foreground">
                <span>Cumulative GDD</span>
                <span className="font-mono text-foreground">
                  {dayResult.cumulative_gdd.toFixed(0)} °C·d
                </span>
              </div>
              <div className="flex justify-between text-muted-foreground">
                <span>fTemp</span>
                <span className="font-mono text-foreground">
                  {dayResult.f_temp.toFixed(2)}
                </span>
              </div>
              <div className="flex justify-between text-muted-foreground">
                <span>Soil Water (PAW)</span>
                <span className="font-mono text-foreground">
                  {dayResult.paw_mm.toFixed(1)} mm (
                  {dayResult.paw_pct.toFixed(0)}%)
                </span>
              </div>
              <div className="flex justify-between text-muted-foreground">
                <span>ETp / ETa</span>
                <span className="font-mono text-foreground">
                  {dayResult.etp.toFixed(1)} / {dayResult.eta.toFixed(1)} mm
                </span>
              </div>
              {simulationData?.yield_estimate_t_ha != null &&
                growthStage === "Harvest Ready" && (
                  <div className="flex justify-between font-semibold text-green-600">
                    <span>Estimated Yield</span>
                    <span>
                      {simulationData.yield_estimate_t_ha.toFixed(2)} t/ha
                    </span>
                  </div>
                )}
            </div>
